...
var VIEW= {
        ...
        if(index == 2){ // 팝업에서 드롭다운 컬럼선택하여 열기직전 호출
          ...
          if(btnId == 'customGridPopBtn3-1'){ 
                if(checkItem.length==0){
                    result.msg = 'Picking Assort 상세에서 분류지시 선택필수!';
                    result.result = 'WARN';
                    return;
                }
                $('#itemIdSP31').val(checkItem[0]['itemId']);
                $('#itemIdSP31').attr("disabled", true);
                $('#locationCdSP31').val(checkItem[0]['locationCd']);
            ...
            if(action='C'&& btnId == 'customBtn31-1'){ 
                for(var i=0,max=param.length; i<max;i++){
                    param[i]['assortId'] = checkItem[0]['assortId'];
                    param[i]['assortDetailId'] = checkItem[0]['assortDetailId'];
    ...
    cellClickCallBack: function(index,rowIndex,target,e) {				
		if(index==0){
			widget.findBtnClicked(1, {assortId:e.item['assortId']}, true, 'CELLCLICK',menuId,VIEW);
		}
		else if(index==1){
			widget.findBtnClicked(2, {assortId:e.item['assortId'],itemId:e.item['itemId']}, true, 'CELLCLICK',menuId,VIEW);
		}
	},
};
$(document).ready(function(event){	
	...
	momWidget.gridPopup.init(3,31,1,'XUDG0310', VIEW);
	VIEW.init();
});
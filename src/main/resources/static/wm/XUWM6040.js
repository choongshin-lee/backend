...
var VIEW= {
  ...
  customCallInit: function(index,your,action,btnId,param,result) {
        if(index == 2){ // 팝업에서 드롭다운 컬럼선택하여 열기직전 호출
          ...
          if(btnId == 'customGridPopBtn3-1'){ 
                if(checkItem.length==0){
                    result.msg = '언로딩 확정 상세에서 언로딩번호 선택필수!';
                    result.result = 'WARN';
                    return;
                }
    ...
    if(index==0){
      ...
      widget.findBtnClicked(2, {unloadingId:e.item['unloadingId'],itemId:e.item['itemId']}, true, 'CELLCLICK',menuId,VIEW);
    }
  },
};
$(document).ready(function(event){  
  ...
  momWidget.gridPopup.init(3,31,1,'XUDG0330', VIEW);
  VIEW.init();
});
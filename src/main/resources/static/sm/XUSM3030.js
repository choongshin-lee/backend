...
var XUSM3030 = {
		...
		var sortMethod    = [{"code":"ASC","value":"오름차순"},{"code":"DESC","value":"내림차순"},{"code":"NONE","value":"사용안함"}];
		  mom_ajax('R', 'XUSM3030.'+queryId, {menuId:prevMenuId,gridId:$('#gridId').val(),programType:$('#programType').val()}, function(result, data) {
		   ...
		   mom_ajax('R', 'XUSM3030.dropDownList', {}, function(result1, data1) {
			 mom_ajax('R', 'XUSM3030.dropDownGridList', {}, function(result2, data2) {
				let comboList =[];
		var columnProperty = [{
			  dataField 	: 'columnId'
			, headerText 	: '컬럼아이디'
	...
	popupGrid: function(queryId) {
		var that = this;
		var searchTypeList  = [{"code":"T","value":"텍스트"},{"code":"I","value":"정수"},{"code":"D","value":"소수점"},{"code":"S","value":"드롭다운싱글"},{"code":"SS","value":"드롭다운싱글(검색)"},{"code":"M","value":"드롭다운멀티"},{"code":"MS","value":"드롭다운멀티(검색)"},{"code":"C","value":"캘린더"},{"code":"C-HM","value":"시분"},{"code":"P","value":"비밀번호"},{"code":"G","value":"그리드"},{"code":"DG","value":"드롭다운그리드"},{"code":"PG","value":"드롭다운그리드(팝업)"},{"code":"FA","value":"파일첨부"},{"code":"DS","value":"비고"}];
		mom_ajax('R', 'XUSM3030.'+queryId, {menuId:prevMenuId,gridId:$('#gridId').val(),programType:$('#programType').val()}, function(result, data) {
			  if(result != 'SUCCESS') {
			      return;
			      momWidget.splashHide();
	...
	buttonGrid: function(queryId) {
		var that = this;
		var buttonTypeList  = [{"code":"createBtn","value":"등록"},{"code":"editBtn","value":"수정"},{"code":"copyBtn","value":"복사"},{"code":"delBtn","value":"삭제"},{"code":"delGridBtn","value":"삭제(그리드)"},{"code":"excelDownBtn","value":"엑셀다운"},{"code":"excelTmpBtn","value":"엑셀양식다운"},{"code":"excelUpBtn","value":"엑셀업로드"},{"code":"excelUpBtnV","value":"엑셀업로드(검사)"},{"code":"addBtn","value":"행추가"},{"code":"moveBtn","value":"이동"},{"code":"saveBtn","value":"저장"},{"code":"fileDownBtn","value":"파일다운"},{"code":"fileDelBtn","value":"파일삭제"},{"code":"customBtn","value":"커스텀버튼"}];
		var searchTypeList  = [{"code":"C","value":"C(create_)"},{"code":"CP","value":"CP(copy_)"},{"code":"U","value":"U(modify_)"},{"code":"CU","value":"CU(upsert_)"},{"code":"D","value":"D(remove_)"},{"code":"R","value":"R(get_)"},{"code":"P","value":"P(proc_)"},{"code":"NONE","value":"없음"}];
		var customTypeList  = [{"code":"ST","value":"표준"},{"code":"CP","value":"커스텀팝업"},{"code":"DG","value":"드롭다운그리드"}];
		mom_ajax('R', 'XUSM3030.'+queryId, {menuId:prevMenuId,gridId:$('#gridId').val(),programType:$('#programType').val()}, function(result, data) {
			  if(result != 'SUCCESS') {
			      return;
			      momWidget.splashHide();
		...
		$(document).on('click', '#saveBtn', function() {
			...
			if(chooseTab == 'button'){
				for(var i=0,max=checkedItems.length;i<max;i++){	
					...
					if(mapList[i].buttonId!='customBtn' && mapList[i].buttonId.indexOf('customBtn')>=0){
						customCnt ++;		

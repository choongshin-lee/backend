    ...
    searchCallInit: function(index,your,action,btnId,param,result,event) { //조회액션 실행 전에 호출되는 함수 
        ...
        if(index==0 ){	                                               
			AUIGrid.clearGridData(widget.grid[1]);	
			    ...
			    result.param = {inoutNo:checkItem[0].returnNo,inoutType:'TD',vendorCd:checkItem[0]['vendorCd'],currencyCd:checkItem[0]['currencyCd']};	
		} 
	    else if(index==20){		
			   result.param = {receiveNo:checkItem[0].receiveNo,inoutType:'TD',vendorCd:checkItem[0]['vendorCd'],currencyCd:checkItem[0]['currencyCd']};	
		}
	},
